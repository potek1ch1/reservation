...
class AuthModel(AbstractModel):
    ...
    # 企業側の会員登録
    def create_office_user(self, username, password):
        hashed_password = self.hash_password(password)
        sql = "INSERT INTO office_users(username, password) VALUE (%s, %s);"
        self.execute(sql, username, hashed_password)
    ...
    # 企業側のログイン処理
    def office_login(self, username, password):
        user = self.find_office_user_by_name_and_password(username, password)
        # 該当するユーザがいなければFalseを返す
        if not user:
            return False, None
        # TODO: セッションに情報を追加
    ...
    def find_office_user_by_name_and_password(self, username, password):
        hashed_password = self.hash_password(password)
        sql = "SELECT * FROM office_users where username=%s AND password=%s"
        return self.fetch_one(sql, username, hashed_password)
